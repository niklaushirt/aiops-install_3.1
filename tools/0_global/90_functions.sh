    ...
    # ---------------------------------------------------------------------------------------------------------------------------------------------------"
        ...
        function __getClusterFQDN() {
            ...
            then
                ...
                IP_HOST=$(ping -c 1 api.$CLUSTER_NAME | sed -n 1p | awk '{print $3}' | ${SED} "s/\://"| ${SED} "s/(//"| ${SED} "s/)//")
 ...
 function printCredentials() {
            ...
            header2Begin "CloudPak for Watson AIOps"
                ...
                header3Begin "CP4WAIOPS"
                        __output "    AIOPS:"
                        __output "        URL:      https://cpd-$WAIOPS_NAMESPACE.$CLUSTER_NAME"

                        __output "        ---------------------------------------------------------------------------------------------"
                        __output "          ADMIN USER:"
                        __output "            User:     $(oc -n ibm-common-services get secret platform-auth-idp-credentials -o jsonpath='{.data.admin_username}' | base64 --decode && echo)"
                        __output "            Password: $(oc -n ibm-common-services get secret platform-auth-idp-credentials -o jsonpath='{.data.admin_password}' | base64 --decode)"
                ...
                header3Begin "Event Manager"
                        ...
                        __output "        Password: $(oc get secret evtmanager-icpadmin-secret -o json -n $WAIOPS_NAMESPACE| grep ICP_ADMIN_PASSWORD  | cut -d : -f2 | cut -d '"' -f2 | base64 --decode;)"

                        __output "---------------------------------------------------------------------------------------------"
                        __output "    SMADMIN USER:"
                        __output "        User:     smadmin"
                        __output "        Password: $(oc get secret evtmanager-was-secret -o json -n $WAIOPS_NAMESPACE| grep WAS_PASSWORD | cut -d : -f2 | cut -d '"' -f2 | base64 --decode;)"
                        
                        __output "---------------------------------------------------------------------------------------------"
                        __output "    ASM TOPOLOGY USER:"
                        __output "        User:     $(oc get secret evtmanager-topology-asm-credentials -n $WAIOPS_NAMESPACE -o=template --template={{.data.username}} | base64 --decode)"
                        __output "        Password: $(oc get secret evtmanager-topology-asm-credentials -n $WAIOPS_NAMESPACE -o=template --template={{.data.password}} | base64 --decode)"
                        __output " "
                        __output " "
                        ...
                        __output "---------------------------------------------------------------------------------------------"
                        __output "    NOI WEBHOOK:"
                        __output "        URL:   <PASTE HERE FOR DOCUMENTATION WHEN CREATED>"
            ...
            header2Begin "Service Now Connection Details"

                        __output "    INTEGRATION SNOW-->CP4WAIOPS:"
                        __output "        URL:                   https://cpd-aiops.$CLUSTER_NAME    (URL for IBM Watson AIOps connection)"
                        __output "        Instance Name:         aimanager"
                        __output "        User:                  admin"
                        __output "        Password:              $(oc -n ibm-common-services get secret platform-auth-idp-credentials -o jsonpath='{.data.admin_password}' | base64 --decode)"
                        __output ""
                        ...
                        __output "    INTEGRATION CP4WAIOPS-->SNOW:"
                        __output "        URL:                   Your SNOW URL"
                        __output "        User ID:               abraham.lincoln             (if you followed the demo install instructions)"
                        __output "        Password:              P4ssw0rd!                   (if you followed the demo install instructions)"
                        __output "        Encrypted Password:    g4W3L7/eFsUjV0eMncBkbg==    (if you followed the demo install instructions)"
                        __output ""

            header2End "Service Now  Connection Details"
            ...
            then
                header2Begin "HUMIO Connection Details"
                            ...
                            __output "        Password: $(oc get secret developer-user-password -n humio-logging -o=template --template={{.data.password}} | base64 --decode)"
                            __output ""
                            __output ""
                            __output ""
            ...
            TURBO_READY=$(oc get pods -n turbonomic | grep "ui" | grep "Running" | grep "1/1" || true) 
            if [[ $TURBO_READY =~ "Running" ]]; 
            then

                header2Begin "Turbonomic Dashboard Connection Details"


                __output "    Turbonomic Dashboard :"
                __output "        URL:      https://api-turbonomic.$CLUSTER_NAME/"
                __output "        User:     administrator"
                __output "        Password: As set at init step"
                header2End "Turbonomic Connection Details"
            fi
        ...
        function checkInstallDone() {
            ...
            AI_MGR_PWD=$(oc -n $WAIOPS_NAMESPACE get secret admin-user-details -o jsonpath='{.data.initial_admin_password}' | base64 --decode || true )     
            while  ([[ $AI_MGR_PWD == "" ]]); do 
                AI_MGR_PWD=$(oc -n $WAIOPS_NAMESPACE get secret admin-user-details -o jsonpath='{.data.initial_admin_password}' | base64 --decode || true )   
                WAIOPS_PODS_COUNT_TOTAL=$(oc get pods -n $WAIOPS_NAMESPACE | grep -v "Completed" | wc -l || true) 
                WAIOPS_PODS_COUNT_NOTREADY=$(oc get pods -n $WAIOPS_NAMESPACE | grep -v "Completed" | grep "0/" | wc -l || true) 
            ...
            COMMON_SVC_PWD=$(oc -n ibm-common-services get secret platform-auth-idp-credentials -o jsonpath='{.data.admin_password}' | base64 --decode || true )     
            ...
            EVT_MGR_PWD=$(oc get secret evtmanager-icpadmin-secret -o json -n $WAIOPS_NAMESPACE| grep ICP_ADMIN_PASSWORD  | cut -d : -f2 | cut -d '"' -f2 | base64 --decode || true )     
        ...
        function checkCSDone() {
            ...
            CS_MONGO_3=$(oc get pods icp-mongodb-0 -n ibm-common-services -oyaml  | grep "phase: Running" || true) 
    ...
    # ------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        function checkStorageClassExists() {
            ...
            then
                ...
                __output "            Please set the correct storage class in file 01_config-install.sh"
                __output "           ❗ Aborting."
                exit 1
            fi
            __output ""
...
function progressbar {
  ...
  printf "\n";
        ...
        function printComponentsInstall() {
            ...
            then
                __output "          ✅ 'Turbonomic' will be installed (needs separate license)"
            else
        ...
        function checkComponentReady() {
            ...
            if [[ $INSTALL_COMPONENT == true ]]; 
            then
                NUM_FOUND=$(oc get pods -n $COMP_NAMESPACE -l $COMP_LABEL_NAME> /dev/null 2>&1 | grep -c "" || true )
        ...
        function checkAlreadyInstalled() {
            ...
            NUM_FOUND=$(oc get pods -n $COMP_NAMESPACE -l $COMP_LABEL >/dev/null 2>&1 | grep -c "" || true )
    ...
    # ----^-----------------------------------------------------------------------------------------------------------------------------------------------"
        ...
        function waitForCPPassword() {
            ...
            while [[ ${FOUND} == "false" && $COUNT -lt $MAX_COUNT ]]; do
                TEMP_PWD=$(oc -n ibm-common-services get secret platform-auth-idp-credentials -o jsonpath='{.data.admin_password}' | base64 --decode || true ) 
...
check_jq() {
    ...
    if [ -x "$(command -v jq)" ]; then
        ...
        __output "      ❗ ERROR: jq is not installed."
        __output "      ❌ Aborting..."
        exit 1
    fi
    __output ""
...
check_kafkacat() {
    __output "   🔎 Check if kafkacat is installed"
    if [ -x "$(command -v kafkacat)" ]; then
        ...
        if [ -x "$(command -v kcat)" ]; then
            ...
            __output "      ❗ ERROR: kafkacat/kcat is not installed."
            __output "      ❌ Aborting..."
            exit 1
        fi
    fi
...
# ---------------------------------------------------------------------------------------------------------------------------------------------------"
# ---------------------------------------------------------------------------------------------------------------------------------------------------"
# Init Code - Kafkacat executable name (kafkacat)
# ---------------------------------------------------------------------------------------------------------------------------------------------------"
# ---------------------------------------------------------------------------------------------------------------------------------------------------"
        # fix sed issue on mac
        OS=$(uname -s | tr '[:upper:]' '[:lower:]')
...
check_elasticdump() {
    __output "   🔎 Check if elasticdump is installed"
    if [ -x "$(command -v elasticdump)" ]; then
        __output "      ✅ OK"
    else
        __output "      ⚠️ WARNING: elasticdump is not installed. "
        __output "         You won't be able to load ES indexes for log training (option 2 in the documentation)"   
        __output "         You won't be able to load ES indexes for SNOW incident training"  
        read -p "       ❗❓ Still continue? [y,N] " DO_COMM
        if [[ $DO_COMM == "y" ||  $DO_COMM == "Y" ]]; then
            __output "      ✅ Ok, continuing..."
        else
            __output "      ❌ Aborting..."
            exit 1
        fi 
...
check_cloudctl() {
    ...
    if [ -x "$(command -v cloudctl)" ]; then
        ...
        __output "      ⚠️ WARNING: cloudctl is not installed. "
        __output "         LDAP Users won't be registered!"  
        read -p "       ❗❓ Still continue? [y,N] " DO_COMM
        if [[ $DO_COMM == "y" ||  $DO_COMM == "Y" ]]; then
            __output "      ✅ Ok, continuing..."
        else
...
check_kubectl() {
    ...
    if [ -x "$(command -v kubectl)" ]; then
        ...
        __output "      ❗ ERROR: kubectl is not installed."
        __output "      ❌ Aborting..."
        exit 1
    fi
    __output ""
...
check_oc() {
    ...
    if [ -x "$(command -v oc)" ]; then
        ...
        if [[ $OC_VERSION =~ "4.7" ]] || [[ $OC_VERSION =~ "4.8" ]] || [[ $OC_VERSION =~ "4.8" ]];
        then 
            __output "      ✅ OK"
        else
            __output "      ⚠️  WARNING: Openshift Client (oc) must be in version 4.7. or higher"
            __output "$OC_VERSION"
            __output "Press Enter to continue"
            read selection
            __output "      ❌ Aborting..."
            exit 1
        fi
    else
        __output "      ❗ ERROR: Openshift Client (oc) is not installed."
        __output "      ❌ Aborting..."
        exit 1
    fi
    __output ""
