...
header2Begin "Input Parameters" "magnifying"
        ...
        if [[ $FLUENTBIT_TOKEN == "" ]];
        then
            __output "     ❌ You must provide the Humio Token for Fluentbit. Aborting...."
            __output "         Usage: $0 -t <TOKEN_FOR_HUMIO_AIOPS_REPOSITORY>"
            
            exit 1
        else
            __output "     ✅ Humio Token for Fluentbit provided: $FLUENTBIT_TOKEN"
        fi
